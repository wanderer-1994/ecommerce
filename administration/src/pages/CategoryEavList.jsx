...
function CategoryEavList (props) {
    ...
    const [eav_list, setEavList] = useState({category_eavs: []});
    useEffect(() => {
        api.getCategoryEavs()
        .then(data => {
    ...
    function changeEavAttribute ({ attribute_id, column, value }) {
        ...
        if (!match) {
            match = eav_list.category_eavs.find(item => item.attribute_id === attribute_id);
    ...
    async function updateEavAttribute (attribute_id, event) {
        ...
        let result = await api.updateCategoryEavs([match]);
        if (result && result.category_eavs && result.category_eavs[0] && result.category_eavs[0].isSuccess) {
            ...
            let m_failure = result && result.category_eavs && result.category_eavs[0] && result.category_eavs[0] ? result.category_eavs[0].m_failure : "";
            appFunction.appAlert({
                icon: "danger",
    ...
    async function deleteEavAttribute (attribute_id, event) {
        ...
        $(event.target).parent().find("button").attr("disabled", true);
        let eav_label;
        let match = eav_list.category_eavs.find(item => item.attribute_id === attribute_id);
        if (match) {
        ...
        appFunction.appAlert({
            ...
            onClickSubmit: async () => {
                let result = await api.deleteCategoryEavs([attribute_id]);
                    ...
                    {category_eav_columns.map((col_item, index) => {
                        ...
                        return (
                            <td style={col_item.td_style} className={`td_input ${col_item.align} ${className}`} key={index}>
                                ...
                                (
                                    <Link to={`/eav/category/${eav_item.attribute_id}`} target="_blank">
                                        <input
        ...
        <div className="category-eav-list">
            ...
            <div className="content">
                <table className="tb_list">
                    ...
                    <tbody>
                        {eav_list && eav_list.category_eavs ? 
                            eav_list.category_eavs.map((eav_item, index) => {
                                if (eav_item){
